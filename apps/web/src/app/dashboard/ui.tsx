...
export function DashboardClient() {
    ...
    async function load() {
      ...
      try {
        const [urlData, installationsData] = await Promise.all([
          ...
          fetchJson<MyInstallationsResponse>("/api/github-app/installations/me"),
        ]);
  ...
  if (loading) {
    return (
      <main className="container flex flex-1 flex-col">
        ...
        <p className="text-muted-foreground">Loading GitHub App integration...</p>
      </main>
    );
  }
  ...
  if (error) {
    return (
      <main className="container flex flex-1 flex-col gap-2">
        <h1 className="text-2xl font-semibold">Dashboard</h1>
    ...
    <main className="container flex flex-1 flex-col gap-4">
      ...
      {!hasInstallations ? (
        <section className="flex flex-col gap-2 rounded-lg border bg-card">
          ...
          <p className="text-muted-foreground">
            No installation found for your account yet. Install the app and select repositories.
          </p>
        ...
        <section className="flex flex-col gap-3">
          ...
          {installations.map((installation) => (
            <article key={installation.id} className="flex flex-col gap-2 rounded-lg border bg-card">
